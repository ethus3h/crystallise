#Based on punk.py
#Version 2, 22 Oct. 2014 and 23 Oct. 2014 a.mn.
...
while running == True:
	...
	if action.lower().strip() == 'save':
		...
		def pack(of):
			...
			timefn = inf+'/.pbz-meta-'+now+'.AddedToPackedDirOnPack.pmb/.Packed-'+now+'.AddedToPackedDirOnPack.ptd'
			timefile.retrieve("http://www.timeapi.org/utc/now?format=%25Y.%25m.%25d.%25H.%25M.%25S.%25Z", timefn)
			tfres = open(timefn,'rb')
			tfres.read()
			tfres.close()
			os.system('hashdeep -c sha1 -r '+inf+' > '+inf+'/.pbz-meta-'+now+'.AddedToPackedDirOnPack.pmb/.Packed-'+now+'.AddedToPackedDirOnPack.pdx')
